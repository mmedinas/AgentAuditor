...
4.  **REMOVER DUPLICATAS:** Se um item da "Fonte B" (texto corrido) já estiver listado na "Fonte A" (listas finais), **NÃO** o repita. A lista final deve ser consolidada e sem duplicatas.
...
---
**IMPORTANTE: APÓS o relatório Markdown, adicione a seção de resumo estruturado para EXPORTAÇÃO:**

[RESUMO ESTRUTURADO PARA EXTRAÇÃO]
| Categoria | Item | Quantidade | Especificacao |
| :--- | :--- | :--- | :--- |
| Elétricos | [Item 1] | [Qtd] | [Breve Espec.] |
| Elétricos | [Item 2] | [Qtd] | [Breve Espec.] |
| Hidráulicos | [Item 3] | [Qtd] | [Breve Espec.] |
* (Repita para CADA item consolidado encontrado. Use 'N/A' se a Qtd ou Espec. não for informada.)
...
# --- NOVA FUNÇÃO para Parsear a Lista Mestra (Extração) ---
def parse_extract_table(summary_section):
    """Parseia a tabela estruturada da função de extração de lista mestra."""
    itens = []
...
.st-emotion-cache-16txtl3 h3, .st-emotion-cache-16txtl3 h6 { /* Seletores podem mudar com versões do Streamlit */
    ...
    color: #0597ff; /* Aplica cor azul aos títulos da sidebar também */
}
/* Tenta garantir que a sidebar tenha um fundo consistente */
[data-testid="stSidebar"] {
    background-color: #ffa804; /* <-- ALTERADO de volta para uma cor neutra */
}
</style>
"""
...
# --- Header ---
st.title("🤖✨ Agente Auditor v6") # <-- ALTERADO para v6
st.caption("Auditoria SP vs. Listas & Extração de Lista Mestra | Gemini Cloud") # <-- ALTERADO


# --- Sidebar (Inputs e Ações) ---
with st.sidebar:
    st.image("https://raw.githubusercontent.com/mmedinas/AgentAuditor/main/LOGO_MOBILE.png", width=150)
    #st.header("⚙️ Controles") # <-- Comentado (conforme seu v5)
    ...
    # Lógica da Chave API (descomentada e corrigida)
        ...
        try:
            ...
            if st.session_state.read_error: st.error(st.session_state.read_error) # Exibe erro de leitura
                ...
                with st.spinner(f"🧠 Extraindo Lista Mestra ({MODEL_NAME})... (Pode levar um tempo)"):
                    ...
                    st.info(f"📡 Enviando {char_count:,} caracteres para a API Gemini...")
                    # 'analysis_content' não é usado, então não precisamos do invoke com dict completo
...
if active_results:
    ...
    st.markdown(f"#### {audit_type}: Relatório Detalhado") # <-- ALTERADO

    # Botão de Download para o Relatório (sempre disponível se houver relatório)
    st.download_button(
         label=f"📄 Baixar Relatório ({audit_type}) (Markdown)", # <-- ALTERADO
    ...
    if isinstance(summary_data, pd.DataFrame) and not summary_data.empty:
        ...
        st.download_button(
            label=f"💾 Baixar Tabela ({audit_type}) (CSV)", # <-- ALTERADO
            data=csv_data,
    ...
    if audit_type == "Auditoria" and isinstance(summary_data, pd.DataFrame) and not summary_data.empty:
        ...
        try:
            ...
            # Removido o expander de diagnóstico (conforme sua solicitação anterior)
    ...
    # Mensagem para 'Extração' (que não tem gráfico)
        ...
        # Opcional: Mostrar a tabela de extração aqui também
        if isinstance(summary_data, pd.DataFrame) and not summary_data.empty:
